...
- **Stream Prefix (префикс для стримов)** - префикс, который будет добавляться к названию стрима при его записи в место назначения. Опциональная настройка, однако по нашей методологии префикс нужно указывать обязательно. Правило формирования префикса: `{название источника}_{название шаблона}_`. Например, если стрим содержит стандартные поля для данного источника данных, то указываем в качестве префикса `{название источника}_default_`. Обычно префикс задаётся в момент создания [[Airbyte Source]], но если в тот момент префикс не был задан, то его можно указать в настройках соединения. #task тут не очень уверена в последнем предложении, нужно уточнить
