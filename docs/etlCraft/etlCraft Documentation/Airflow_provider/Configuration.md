...
| `templated_file`     | Значение берется из файла, лежащего в проекте dbt в папке `templated_configs`. Файл должен называться `{название конфига}{путь}`, в конце может быть приписка `_manual`.  Этой папкой управляет отдельный DAG [[template_configs]].                                                    | Когда нужно взять преднастроенное значение конфига из **etlCraft**, при этом дать возможность пользователю перезаписать это значение                                                               | Пустой, что означает, что конфиг будет взят из проекта dbt, папке `templated_configs` в файле `{ название конфига}_manual` или `{ название конфига}`. Если задать путь `_v2`, то конфиг будет искаться в файлах `{ название конфига}_v2_manual` или `{ название конфига}_v2`. |
