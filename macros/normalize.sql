...
{%- if execute -%}
    ...
    {%- for key in column_set -%}
        {%- do column_list.append(etlcraft.json_extract_string('_airbyte_data', key) ~ " AS " ~ etlcraft.normalize_name(key)) -%}
    {%- endfor -%}

    SELECT
        _dbt_source_relation AS _table_name,        
        _airbyte_emitted_at AS _emitted_at,
