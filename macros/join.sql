...
{%- set relations_dict =  etlcraft.get_relations_dict('incremental',sourcetype_name,pipeline_name,stream_name) -%}

{%- if not disable_incremental %}
{#- получаем список date_from:xxx[0], date_to:yyy[0] из union всех normalize таблиц -#}
  {%- set min_max_date_dict = etlcraft.get_min_max_date('normalize',sourcetype_name,pipeline_name,stream_name) -%}
...
{% set macro_name =  'join_'~ sourcetype_name ~'_'~ pipeline_name ~'_'~ stream_name %}
{%- endif -%}

{#- здесь перечислены параметры, которые д.б. такими же в каждом виде макроса join -#}
{{ etlcraft[macro_name](sourcetype_name,pipeline_name,stream_name,relations_dict,date_from,date_to,params)}}
