...
Для пайплайна `events` делаем материализацию `table` и соединяем данные из таблиц `link_events` + `graph_qid` + имеющиеся таблицы пайплайна `registry`.

Для пайплайна `datestat` делаем материализацию `incremental` и соединяем данные из таблицы `link_datestat` + имеющиеся rтаблицы пайплайна `registry`.
